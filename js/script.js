$(document).ready(function() {
    ...
    $("button").click(function() {      // CLICKING BUTTON
            $("button").css({
                pointerEvents: "none"
            })
            setTimeout(() => {
            ...
            if($(this).attr("id") === "rock") {                 // CHOOSE ROCK
                changingImages(0);
                if(random === 0) {              // DRAW GAME
                    tableLogic("rock", "rock", "draw");
                }
            ...
            function changingImages(id) {       // CHANGE IMAGES
                $("img#player").attr("src", comp[id].src);
                $("img#computer").attr("src", comp[random].src);
            }
            ...
            function endGame() {    // END GAME
                $("<div class='overlay'><button class='btn btn-outline-primary'>PLAY AGAIN</button></div>").appendTo("body");
